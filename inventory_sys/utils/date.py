...
def get_low_stock_items():
    ...
    critical_threshold = 0.2  # Consider moving to settings
...
def get_top_selling_products(orders, buying_price_subquery):
    ...
    try:
        ...
        logger.error("Database error in top_selling query: %s", str(e))
        return []
...
def fetch_report_data(request):
    """Fetches and returns report data as JSON."""
    try:
        # Initialize dates
        ...
        total_cogs, gross_profit, gross_margin = calculate_cogs_and_gross_profit(orders, total_revenue, buying_price_subquery)
        ...
        data = {
            ...
            'default_start_date': thirty_days_ago.isoformat(),
        ...
        return JsonResponse({'error': 'An error occurred while fetching report data'}, status=500)
